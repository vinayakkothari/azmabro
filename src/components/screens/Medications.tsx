...
export default function Medications() {
  const { navigate, medications, toggleMed } = useApp()
  const nonRescue = medications.filter(m => !m.isRescue)
  const taken     = medications.filter(m => m.taken && !m.isRescue)
    ...
    <div className="screen active">
      <div className="scr-hdr">
        ...
        <div className="scr-sub">
          Today · {taken.length} of {nonRescue.length} taken — nice!
        </div>
      </div>
      ...
      <div className="scroll">
        <div className="meds-body">
          ...
          {medications.map(med => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="med-lbl" style={med.isRescue ? { color: '#E53E3E' } : {}}>
                  {med.label}
                </div>
          ...
          <div style={{ marginTop: 20 }}>
            <div className="sec">Adherence This Week</div>
            <div className="card" style={{ padding: 15 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--charcoal)' }}>85% adherence</span>
                <span style={{ fontSize: 12, color: 'var(--success)', fontWeight: 600 }}>🌟 Great work!</span>
              </div>
              <div className="adh-bar-bg">
                <div className="adh-bar-fill" style={{ width: '85%' }} />
              </div>
            </div>
          </div>
        </div>
